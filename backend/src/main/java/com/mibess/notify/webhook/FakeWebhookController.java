...
public class FakeWebhookController {
  ...
  @PostMapping("/api/v1/admin/notifications/{id}/simulate-delivery")
  @PreAuthorize("hasRole('ADMIN')")
  public void simulate(@PathVariable UUID id) {
    var n = db
      .sql(
        "SELECT channel_connection_id,provider_message_id FROM notifications WHERE id=:id AND workspace_id=:w AND status='SENT'"
      )
      .param("id", id)
      .param("w", access.id())
      .query()
      ...
      .findFirst()
      .orElseThrow(() ->
        new IllegalArgumentException("Notificação deve estar SENT")
      );
    var c = store.get(
      Kind.CHANNELS,
      access.id(),
    ...
    if (
      !c.spec().path("provider").asText().equals("FAKE")
    ) throw new IllegalArgumentException("Simulação exige provider fake");
    var value = json.tree(
      Map.of(
        "statuses",
        List.of(
