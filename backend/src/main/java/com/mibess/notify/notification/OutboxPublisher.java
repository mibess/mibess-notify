...
public class OutboxPublisher {
  ...
  @Scheduled(fixedDelayString = "${notify.outbox-delay:1000}")
  public void publish() {
    tx.executeWithoutResult(status -> {
      var rows = db
        .sql(
          "SELECT * FROM outbox WHERE published_at IS NULL ORDER BY created_at LIMIT 50 FOR UPDATE SKIP LOCKED"
        )
        .query()
        .listOfRows();
      for (var row : rows) {
        ...
        p.setMessageId(row.get("id").toString());
        p.setPriority(((Number) row.get("priority")).intValue());
        try {
          rabbit.send(
            "notify",
          ...
          db.sql("UPDATE outbox SET published_at=now() WHERE id=:id")
            .param("id", row.get("id"))
            .update();
        } catch (Exception e) {
          break;
