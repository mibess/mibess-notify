...
public class Crypto {
  ...
  public Crypto(@Value("${notify.encryption-key}") String key) {
    ...
    if (masterKey.length != 32) throw new IllegalArgumentException(
      "MASTER_ENCRYPTION_KEY deve conter 32 bytes em Base64"
    );
  }
  ...
  public String encrypt(String plain, String context) {
    try {
      ...
      throw new IllegalStateException("Falha na criptografia");
    }
  }
  ...
  public String decrypt(String encrypted, String context) {
    try {
      ...
      if (
        parts.length != 3 || !parts[0].equals("v1")
      ...
      c.updateAAD(context.getBytes(StandardCharsets.UTF_8));
      return new String(
        c.doFinal(Base64.getDecoder().decode(parts[2])),
        StandardCharsets.UTF_8
      );
    } catch (Exception e) {
      throw new IllegalStateException("Credencial indisponível");
    }
  }
  ...
  public static boolean signature(
    ...
    if (
      signature == null || !signature.matches("sha256=[0-9a-fA-F]{64}")
    ) return false;
    try {
